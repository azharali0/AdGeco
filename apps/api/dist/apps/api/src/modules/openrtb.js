...
export async function registerOpenRtbRoutes(app, tokenPepper) {
    app.post('/openrtb/2.6/bid', async (req, reply) => {
        const apiKey = String(req.headers['x-openrtb-key'] ?? '');
        if (!apiKey)
            return reply.code(401).send({ code: 'OPENRTB_AUTH_REQUIRED' });
        const prefix = apiKey.slice(0, 16);
        const registration = await prisma.sdkRegistration.findFirst({ where: { publicKeyPrefix: prefix, status: { in: ['CERTIFIED', 'ACTIVE'] } } });
        if (!registration || registration.secretHash !== hashToken(`${tokenPepper}:${apiKey}`))
            return reply.code(401).send({ code: 'INVALID_OPENRTB_CREDENTIAL' });
