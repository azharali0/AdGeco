...
export function authorise(ctx, permission) {
    const allowed = permissions[permission];
    if (!ctx.roles.some((role) => allowed.includes(role)))
        throw new Error(`FORBIDDEN:${permission}`);
}
export function assertTenant(ctx, organisationId) {
    if (ctx.organisationId !== organisationId && !ctx.roles.includes('SUPER_ADMIN'))
...
export function generateTotpSecret() { let out = ''; const bytes = randomBytes(20); for (const byte of bytes)
    out += BASE32[byte % 32]; return out; }
function decodeBase32(value) { let bits = ''; for (const char of value.replace(/=+$/, '').toUpperCase()) {
...
export function totp(secret, time = Date.now(), step = 30) { const counter = Math.floor(time / 1000 / step); const buffer = Buffer.alloc(8); buffer.writeBigUInt64BE(BigInt(counter)); const digest = createHmac('sha1', decodeBase32(secret)).update(buffer).digest(); const offset = digest[digest.length - 1] & 15; const code = ((digest[offset] & 127) << 24) | ((digest[offset + 1] & 255) << 16) | ((digest[offset + 2] & 255) << 8) | (digest[offset + 3] & 255); return String(code % 1_000_000).padStart(6, '0'); }
export function verifyTotp(secret, code, now = Date.now()) { return [-1, 0, 1].some((w) => { const expected = totp(secret, now + w * 30_000); return expected.length === code.length && timingSafeEqual(Buffer.from(expected), Buffer.from(code)); }); }
export function encryptSecret(value, pepper) { const key = createHash('sha256').update(pepper).digest(); const iv = randomBytes(12); const cipher = createCipheriv('aes-256-gcm', key, iv); const ciphertext = Buffer.concat([cipher.update(value, 'utf8'), cipher.final()]); const tag = cipher.getAuthTag(); return [iv, tag, ciphertext].map((b) => b.toString('base64url')).join('.'); }
export function decryptSecret(value, pepper) { const [iv, tag, ciphertext] = value.split('.').map((v) => Buffer.from(v, 'base64url')); const key = createHash('sha256').update(pepper).digest(); const decipher = createDecipheriv('aes-256-gcm', key, iv); decipher.setAuthTag(tag); return Buffer.concat([decipher.update(ciphertext), decipher.final()]).toString('utf8'); }
