...
import { ConsoleEmailProvider, HttpEmailProvider, type EmailProvider } from '@adgeco/email';
...
const emailProvider:EmailProvider=process.env.EMAIL_API_URL&&process.env.EMAIL_API_KEY&&process.env.EMAIL_FROM
  ? new HttpEmailProvider(process.env.EMAIL_API_URL,process.env.EMAIL_API_KEY,process.env.EMAIL_FROM)
  : new ConsoleEmailProvider();

type ClaimedOutbox={id:string;organisationId:string|null;type:string;payload:unknown;attempts:number};

async function deliverWebhook(delivery:{id:string;endpointId:string;eventType:string;payload:unknown;attempts:number;endpoint:{url:string;secretHash:string}}){
...
async function processWebhooks(){
 ...
 for(const row of rows){
  try{await deliverWebhook(row as never);}
  catch(error){const attempts=row.attempts+1;await prisma.webhookDelivery.update({where:{id:row.id},data:{status:attempts>=8?'FAILED':'RETRYING',attempts,lastError:String(error),nextAttemptAt:new Date(Date.now()+Math.min(3600,2**attempts*5)*1000)}});}
 }
}

async function claimOutbox():Promise<ClaimedOutbox[]>{
 return prisma.$queryRawUnsafe<ClaimedOutbox[]>(`
  WITH candidates AS (
...
async function dispatchEmail(row:ClaimedOutbox){
 if(typeof row.payload!=='object'||!row.payload)return;
 const payload=row.payload as Record<string,unknown>;
 if(typeof payload.email!=='string')return;
 if(!tokenPepper)throw new Error('TOKEN_PEPPER_REQUIRED');
 if(row.type==='UserRegistered'){
  if(typeof payload.verificationTokenEncrypted!=='string')throw new Error('EMAIL_VERIFICATION_PAYLOAD_INCOMPLETE');
 ...
 if(row.type==='PasswordResetRequested'){
  if(typeof payload.resetTokenEncrypted!=='string')throw new Error('PASSWORD_RESET_PAYLOAD_INCOMPLETE');
 ...
 if(row.type==='OrganisationInvitationCreated'){
  if(typeof payload.invitationTokenEncrypted!=='string')throw new Error('INVITATION_PAYLOAD_INCOMPLETE');
...
async function processOutbox(){
 ...
 for(const row of rows){
  try{
   ...
   for(const endpoint of endpoints)await prisma.webhookDelivery.upsert({where:{endpointId_eventId:{endpointId:endpoint.id,eventId:row.id}},create:{endpointId:endpoint.id,eventId:row.id,eventType:row.type,payload:row.payload as object,status:'PENDING',nextAttemptAt:new Date()},update:{}});
...
async function main(){logger.info('worker_started',{workerId});while(true){await Promise.allSettled([processOutbox(),processWebhooks(),expireReservations(),heartbeat()]);await sleep(Number(process.env.WORKER_POLL_MS??1000));}}
